...
# padding helper function added for CSCI 662
def pad_to_length(tokens, pad_len, pad_id):
    return tokens[:pad_len] + [pad_id] * max(0, pad_len - len(tokens))
...
class BPETokenizer:
    ...
    def __init__(self):
        ...
        # added for CSCI 662: include a pad token by default
        if '<PAD>' not in self.encoder.encoder:
            self.pad_id = max(self.encoder.encoder.values()) + 1
            self.encoder.encoder['<PAD>'] = self.pad_id
            self.encoder.decoder[self.pad_id] = '<PAD>'
        else:
